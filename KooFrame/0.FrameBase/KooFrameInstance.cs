...
namespace KooFrame.BaseSystem
{
	public static class KooFrameInstance
	{
		...
		)
		{
			...
			if (isAddDataPath)
			{
				sb.Append(KooFileTool.GetLocalPathTo());
			}
		...
		public static void GeneratorUnityPackage()
		{
			...
			KooFileTool.ExportPackage(ExtralEditorUtility.FindFoldersWithEndingPath("/KooFrame")[0], GetFrameworkFileName(isAddDataPath: true, isAddAssetsBefore: false, isAddVersion: true, isAddTime: true,
				isAddFileType: true));
			KooFileTool.OpenLocalPath();
