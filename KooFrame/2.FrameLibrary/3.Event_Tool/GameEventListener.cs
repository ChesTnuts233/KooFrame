...
namespace KooFrame
{
    ...
    public class GameEventListener : MonoBehaviour, IMouseEvent
    {
        ...
        {
            public Dictionary<int, IGameEventListenerEventInfos> eventInfoDic = new Dictionary<int, GameEventListener.IGameEventListenerEventInfos>();
        }
        ...
        private class GameEventListenerEventInfos<T> : IGameEventListenerEventInfos
        {
            ...
            {
                GameEventListenerEventInfo<T, TEventArg> info = poolModul.GetObject<GameEventListenerEventInfo<T, TEventArg>>();
            ...
            public void RemoveListener<TEventArg>(Action<T, TEventArg> action, TEventArg args = default(TEventArg))
            {
                for (int i = 0; i < eventList.Count; i++)
                {
                    GameEventListenerEventInfo<T, TEventArg> eventInfo = eventList[i] as GameEventListenerEventInfo<T, TEventArg>;
                    if (eventInfo == null) continue; // 类型不符
