...
namespace GridSystem
{
    public class GridMapXZ<TGridObject>
    {
        ...
        private static bool isDebugGrid = false;
        ...
        public GridMapXZ(int width, int height, float cellSize, Vector3 originPosition)
        {
            ...
            if (isDebugGrid)
            {
                for (int x = 0; x < GridTArray.GetLength(0); x++)
                {
                    ...
                    {
                        // debugTextArray[x, z] = UtilsClass.CreateWorldText(GridTArray[x,z] == null? "NULL": GridTArray[x,z].ToString(), GameObject.Find("GridBuild").transform,
                        //     GetWorldPosition(x, z) + new Vector3(cellSize, 0, cellSize) * .5f, 4, Color.green,
